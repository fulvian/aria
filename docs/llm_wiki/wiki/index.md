...
| `docs/plans/stabilizzazione_aria.md` | Piano stabilizzazione pre-Fase 2 ripristinato come source of truth (F0-F5, rollback-first) | 2026-04-30 |
...
| Source | Description | Last Updated |
|--------|-------------|--------------|
| `.aria/config/agent_capability_matrix.yaml` | Capability matrix YAML canonica (4 agenti) | 2026-04-30 |
...
| `src/aria/agents/coordination/registry.py` | YAML-backed agent registry (loader + validator helpers) | 2026-04-30 |
...
| Source | Description | Last Updated |
|--------|-------------|--------------|
| `.aria/config/mcp_catalog.yaml` | Catalogo MCP canonico (14 server catalogati, lista YAML) | 2026-04-30 |
| `src/aria/mcp/capability_probe.py` | Probe generalizzato che supporta YAML catalog + runtime `mcp.json` | 2026-04-30 |
| `src/aria/launcher/lazy_loader.py` | Lazy bootstrap per intent compatibile con il catalogo YAML reale | 2026-04-30 |
| `scripts/check_mcp_drift.py` | Entrypoint compatibile che delega a `scripts/audit_drift.py` | 2026-04-30 |
...
| Source | Description | Last Updated |
|--------|-------------|--------------|
| `.aria/kilocode/mcp.json` | MCP server runtime config (14 enabled + 1 disabled; include compat `git`/`github`) | 2026-04-29 |
...
| **[[observability]]** | **NEW v5.0**: L4 — Logger structured JSON, Prometheus metrics, eventi tipati, trace_id via contextvars | **✅ v1.0** |
...
- **ADR presenti nel repository**: ADR-0001..0008; le ADR 0009/0010/0012/0013/0014 non risultano ancora materializzate come file dedicati.
- **Quality gate storico v5.0**: vedi `docs/llm_wiki/wiki/log.md` per i run registrati il 2026-04-30; la remediation corrente ha verificato i moduli toccati con test mirati.
